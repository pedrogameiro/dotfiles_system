...
function search_headsets() {
	...
	for mac in `bt-device -l 2>/dev/null | perl -ne '/.*\((.*)\)/ and print "$1\n"'`
	do
		# search for connected device with AudioSink service
		if [[ `bt-device -i $mac 2>/dev/null | perl -00 -ne '/.*Trusted: 1.*\n\s*Blocked: 0.*\n\s*Connected: 1\n\s*UUIDs: .*AudioSink.*/ and print "1\n"'` ]]; then

            # WH-1000XM2
            if [ "$mac" = '70:26:05:29:55:B2' ]; then
